...
216=>array("nama"=>"JAKARTA", "discount"=>array( 0 , 1.5 ), "minimal_order"=>200 ),
 ...
 $sql = "select c.* from [order] a inner join  ". $GLOBALS["database_accpac"] ."..ARCUS b on A.dealer_id = b.IDCUST 
			...
			where a.order_id = '". main::formatting_query_string( $data_dealer["order_id"] ) ."'";
 ...
 if( sqlsrv_num_rows( $rs_teritori_dealer ) > 0 && @$teritori_dealer["territory"] != "" && array_key_exists( $teritori_dealer["territory"], $arr_teritori_diskon_kontainer ) ){
	...
	if( $nominal_order["nominal_order"] >= ( $teritori_dealer["territory"]["minimal_order"] * 1000000 ) ){ // konversi minimal order ke satuan juta rupiah
		$level1 = $nominal_order["nominal_order"] * $arr_teritori_diskon_kontainer[ $teritori_dealer["territory"] ]["discount"][0] / 100 ;
		$level2 = $level1 * $arr_teritori_diskon_kontainer[ $teritori_dealer["territory"] ]["discount"][1] / 100;
	}
