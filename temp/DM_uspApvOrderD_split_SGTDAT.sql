...
/****** Object:  StoredProcedure [dbo].[DM_uspApvOrderD_split_SGTDAT]    Script Date: 11/24/2017 16:08:09 ******/
SET ANSI_NULLS ON
GO
...
BEGIN TRANSACTION
	...
	declare @diskon_per_invoice decimal(19,3), @keterangan_per_item varchar(60), @codeterr varchar(10), @kategori_gift varchar(10), @lokasi_asal varchar(10), @acctset varchar(5), @approval_user varchar(50)
	...
	from 
	dbo.ufn_daftar_order_item_split_disetujui(@orderid) a
	...
	select 
		...
		,e.codeterr, isnull(a.kategori_gift, '') kategori_gift, f.gudang lokasi_asal, b.cntlacct, case when isnull(g.approval_user, '') = '' then d.approval_user else g.approval_user end
		from
dbo.ufn_daftar_order_item_split_disetujui( @orderid )
	...
	left outer join (select distinct kode_sales, approval_user from [user] where approval_user is not null ) g on g.kode_sales = d.bm
