...
class akumulasi_kuantitas_order extends sql_dm{
	
	public 
		$akumulasi_kuantitas_order = 0,
	...
	function __construct( $obyek_dm, $arr_parameter ){
		...
		$sql = "select a1.* from (
					select SUM(e.QTYSHIPPED)-SUM(ISNULL( g.qtyreturn, 0 )) kuantitas, SUM(e.INVDISC) diskon, sum(e.EXTINVMISC - e.INVDISC - e.HDRDISC  - isnull(g.EXTCRDMISC, 0) - isnull(g.CRDDISC, 0) - isnull(g.HDRDISC, 0)) subtotal_order, c.order_id, c.user_id
				...
				a1, [order] a2 where a1.order_id = a2.order_id and a1.user_id = a2.user_id
		...
		while( $data_invoice = sqlsrv_fetch_array( $rs_data_invoice ) ){
			...
			$this->akumulasi_kuantitas_order += $data_invoice["kuantitas"];
