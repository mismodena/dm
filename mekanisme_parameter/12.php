/*select 
sum(a.harga*a.kuantitas) nilai_order
from order_item a, [order] b, paket_item c where 
a.order_id = b.order_id and a.user_id = b.user_id and a.item_id = c.item and a.paketid = c.paketid and c.paketid = '#paketid#'
and a.order_id = '#order_id#' and a.user_id = '#user_id#'
======================================================
select sum(nilai_order) from(
	/* ITEM SINGLE */
	select 
		sum(a.kuantitas * a.harga) nilai_order
		from order_item a, [order] b, paket_item c where 
		a.order_id = b.order_id and a.user_id = b.user_id and a.item_id = c.item and a.paketid = c.paketid and c.mode = '1' and
