<?
	...
	$notPPOK = array('A3','A4','A7','V','Y01','Y02','Y11','Y12','Z','Z1');
	$urlp = "Salah";
	if (!in_array(trim($data_dealer["idgrp"]),$notPPOK)){
		if(trim($data_dealer["idgrp"])=='A1' and $data_dealer["codeterr"]<>221) {
				$urlp = "";
		}
		else{
			if($status_order == 1){
				...
				$nama = trim($data_dealer["nama_lengkap"]);
				$ket = trim($data_dealer["keterangan_order"]);
				$email = trim($data_dealer["email"]);
				$emailbm = trim($data_dealer["email_bm"]);
