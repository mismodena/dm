...
class mekanisme_prosedur_diskon_13 extends prosedur_khusus_tambahan_diskon{

	private 
		$mekanisme_prosedur_diskon = "",
		$arr_diskon_id_saling_melengkapi = array(1,13,14),
		$arr_diskon_id_share_budget = array(13,14),
	...
	function __construct( $arr_parameter, $readonly = "", $parameter_template = "", $arr_budget_diskon_tersedia_terkait = array() ){
		...
		$budget_terpakai_non_share = $pemakaian_bq_freeitem + $pemakaian_bq_diskon;
		$arr_rpl["#budget_terpakai_non_share#"] =  "<span style=\"border-bottom:3px red none\">" . 
						self::number_format_dec( $pemakaian_bq_freeitem ) . "<sup> [1]</sup></span> + ".  
						self::number_format_dec( $pemakaian_bq_diskon ) . "<sup> [2]</sup>";
		$arr_rpl["#keterangan_bqtq#"] = "<div style=\"font-size:11px; float:left\">
			<sup>[1]</sup>. Pemakaian free item BQ yang sedang dalam proses pengajuan<br />
			<sup>[2]</sup>. Pemakaian tambahan diskon potong BQ yang sedang dalam proses pengajuan<br />
			<sup>[3]</sup>. Akumulasi pemakaian saldo BQ dari order lain yang sedang dalam proses pengajuan (Free item BQ + Tambahan diskon potong BQ)</div>";
		...
		while( $daftar_pemakaian_bqtq = sqlsrv_fetch_array( $rs_daftar_pemakaian_bqtq ) ){
			...
			if( $daftar_pemakaian_bqtq["nilai_diskon"] != "" ) $arr_diskon_diterapkan[] = $daftar_pemakaian_bqtq["diskon_id"];
		}
		...
		$saldo_budget = self::saldo_bqtq( $arr_parameter["a3.dealer_id"][1], /*$arr_parameter["a.order_id"][1]*/"" )[ $this->prefiks_identifikasi_bqtq . "Avail"]  * $this->persentase_budget_bisa_digunakan;
		...
		if( $free_item_bq_ada ){
			...
			$rs_daftar_pemakaian_bq_order_ini = sqlsrv_fetch_array( 
					prosedur_khusus_tambahan_diskon::daftar_order_diskon_bqtq( "", $arr_parameter__, "SUM(a.diskon_bqtq) / 0.7 total_nilai_diskon", "  " )
				);		
		}
		...
		foreach($this->arr_diskon_id_sebudget as $diskon_id){
			...
			$daftar_pemakaian_bq_order_lain += ( $rs_daftar_pemakaian_bqtq_order_lain["total_nilai_diskon"] / ( $diskon_id == 1 ? 0.7 : 1 ) );
		}

		//$arr_rpl["#budget_terpakai_non_share#"] = self::number_format_dec( $rs_daftar_pemakaian_bqtq_non_share["total_nilai_diskon"] ) . ( $free_item_bq_ada ? " + " . self::number_format_dec( @$rs_daftar_pemakaian_bq_order_ini["total_nilai_diskon"] ) : "" );
		//$arr_rpl["#saldo_bqtq#"] = self::number_format_dec( $saldo_budget + round( $proporsi_diskon ) + $rs_daftar_pemakaian_bqtq_non_share["total_nilai_diskon"] +  @$rs_daftar_pemakaian_bq_order_ini["total_nilai_diskon"] + $daftar_pemakaian_bq_order_lain );//@$rs_daftar_pemakaian_bqtq_order_lain["total_nilai_diskon"]);
		//$arr_rpl["#budget_terpakai_order_lain#"] = self::number_format_dec( $daftar_pemakaian_bq_order_lain );

		//$this->saldo_tersedia_akhir = $saldo_budget ;//+ @$rs_daftar_pemakaian_bq_order_ini["total_nilai_diskon"];
